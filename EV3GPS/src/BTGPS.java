...
import lejos.hardware.gps.GPS;
import lejos.hardware.gps.Satellite;
...
public class BTGPS{
	...
	//Bluetooth
	private static RemoteBTDevice GPSDevice = null;
		...
		if(GPSDetected){
			...
			if(connectionStatus == 2){
				...
				if(connectionStatus == -1){
					...
					LCD.drawString("Connection error", 0, 7);
				}
	...
	static boolean discoverBTDevices(){
		...
		LCD.drawString("Search done", 0, 2);
		LCD.drawInt(devList.size(), 2, 0, 3);
		LCD.refresh();

		//If exist GPS Devices near
		if (devList.size() > 0) {
			RemoteBTDevice[] devices = devList.toArray(new RemoteBTDevice[devList.size()]);
			String[] names = new String[devList.size()];
			
			for (int i = 0; i < devList.size(); i++) {
				RemoteBTDevice btrd = devices[i];
			...
			do {
				...
				if (selected >=0){
					RemoteBTDevice btrd = devices[selected];
					LCD.clear();
					LCD.drawString("Found",6,0);
					LCD.drawString(names[selected],0,1);
					LCD.drawString(btrd.getAddress(), 0, 2);
	...
	static int connectGPS(){
		...
		LCD.drawString(GPSDevice.getName(), 0, 1);
		LCD.refresh();
